...
# get load averages
IFS=" " read LOAD1 LOAD5 LOAD15 <<<$(cat /proc/loadavg | awk '{ print $1,$2,$3 }' | lolcrab -S 100 -s 0.01)
# get free memory
IFS=" " read USED AVAIL TOTAL <<<$(free -htm | grep "Mem" | awk {'print $3,$7,$2'} | lolcrab -S 100 -s 0.01)
# get processes
...
W="\e[0;39m"
G="\e[1;32m"
figlet -w 50 -c `hostname -s` | lolcrab -S 100 -s 0.01

echo "  [SYSTEM]--------------------------------------------" | lolcrab -S 100 -s 0.01
echo -e "  
  Hostname....: `hostname -s`
...
#printf "\ndisk usage:\n"
echo "  [STORAGE]-------------------------------------------" | lolcrab -S 100 -s 0.01
echo -e ""
#printf "  [STORAGE]\n"
for line in "${dfs[@]}"; do
    # get disk usage
    ...
    echo -e "${bar}" | sed -e 's/^/  /' | lolcrab -S 100 -s 0.01
done
printf "\n"
